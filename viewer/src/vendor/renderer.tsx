...
// packages/tui/src/routes/session/index.tsx, lines 157–176 (the display
// context) and 1365–2725 (the pure renderer half: message components, the
// part dispatch, the tool registry). The interactive shell half stays
// upstream. Deliberate divergences, each marked "viewer:" inline:
//   - imports point at the viewer's store/theme/config and vendored utils
//   - ReasoningPart uses the shared subtle syntax style singleton
//   - the subagent hint/keybinding line under task tools is dropped
// Resync against upstream deliberately; do not restyle in passing.
import { createMemo, createSignal, createContext, useContext, For, Match, onMount, Show, Switch } from "solid-js"
...
import { selectedForeground } from "./theme-core"
...
import { setPreLayoutSiblingMargin } from "./layout"
import { normalizePath } from "./path"
import * as Model from "./model"
import { reasoningSummary, type ThinkingMode } from "./thinking"
import { webSearchProviderLabel } from "./tool-display"
import { useLocal, usePathFormatter, useTuiTerminalEnvironment, useRoute, useDialog, DialogAlert } from "./shims"
...
const context = createContext<{
  ...
  tui: typeof tuiConfig
}>()

export const TranscriptContext = context

function use() {
  const ctx = useContext(context)
  if (!ctx) throw new Error("useContext must be used within a TranscriptContext provider")
  return ctx
}
...
export function AssistantMessage(props: { message: AssistantMessageInfo; parts: Part[]; last: boolean }) {
  ...
  return (
    <>
      ...
      {/* viewer: upstream renders a "view subagents" keybinding hint here; the
          viewer has no subagent navigation, so the hint would advertise a dead
          key and is dropped. */}
...
function ReasoningPart(props: { last: boolean; part: ReasoningPartInfo; message: AssistantMessageInfo }) {
  ...
  // Reasoning is finalized when time.end is set; flips independently of the
  // parent message completing.
  ...
  // viewer: upstream builds a per-part subtle SyntaxStyle memo; the viewer's
  // theme is static so the shared singleton serves every reasoning part.
  const syntax = subtleSyntax
...
function InlineTool(props: {
  ...
  const permission = createMemo(() => {
    const callID = (sync.data.permission[ctx.sessionID] as { tool?: { callID?: string } }[] | undefined)?.at(0)?.tool
      ?.callID
  ...
  const tools = createMemo(() => {
    return messages().flatMap((msg) =>
      (sync.data.part[msg.id] ?? [])
        .filter((part): part is ToolPartInfo => part.type === "tool")
