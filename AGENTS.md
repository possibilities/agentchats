...
- `scripts/install.sh` — the installation contract for cass
  ...
  then build or refresh the index so Claude Code and Codex sessions are
  searchable.
...
- Installer-owned Cass commands run against the canonical HOME, config, and
  data boundary with provider-root overrides removed. Before the first command,
  every data-path component is proved canonical, operator-owned, and not
  group/world writable. Index and search children are time-bounded and fully
  reaped on timeout or installer termination.
