...
const conversation =
  /^\s*<realtime_conversation>([^<>]*)<\/realtime_conversation>\s*$/
...
export function parseCodexMessagePresentation(
  content: string,
): MessagePresentation | undefined {
  const match = delegation.exec(content)
  if (match) {
    const source = match[1]?.trim()
    if (match[1] !== undefined && source !== "transcript_tail_flush")
      return undefined
    ...
    if (source === "transcript_tail_flush") {
      return {
        ...
        details: [
          ...(transcript
            ? [{ label: "Voice context", content: transcript }]
            : []),
