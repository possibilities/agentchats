...
export function DocumentViewerProvider({
  ...
  const accepts = useCallback(
    (request: DocumentRequest) =>
      isLocalMarkdownHref(request) && canOpen(request),
    [canOpen],
  )

  const open = useCallback((request: DocumentRequest, trigger: HTMLElement) => {
    if (history.length === 0) opener.current = trigger
    setView((state) => {
      const previous = state.history.at(-1)
      if (
        previous?.request.href === request.href &&
        previous.request.base === request.base
      )
        return state
  ...
  useEffect(() => {
    ...
    void load({ ...request, signal: controller.signal }).then(
      (document) => {
        ...
        setView((state) => {
          const entry = state.history.at(-1)
          if (!entry || entry.request !== request || entry.attempt !== attempt)
            return state
          return {
            ...state,
            history: [
              ...state.history.slice(0, -1),
              { ...entry, status: "ready", document },
            ],
          }
        })
      },
  ...
  const title = current?.document?.title ||
    (current ? displayName(current.request.href) : "Document")
          ...
          >
            <header className="document-viewer__header">
              <div className="document-viewer__heading">
                <Dialog.Title className="document-viewer__title">
                  {title}
                </Dialog.Title>
