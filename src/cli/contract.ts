...
const commands: ContractCommand[] = [
  ...
  {
    name: "routing", summary: "Inspect routing choices and native configuration from exact Codex transcripts",
    audience: "agent", mutates: false, x_output: "json",
    guidance: "Reads original uncompressed Codex rollout files on demand, without an index, native RPC, directory scan or retained copy. Supply exact related child paths to join native ancestry and child turn configurations. Child thread history is not per-assignment or backend execution proof. Missing decisions and acceptance remain unknown; receipts are authored claims. Output is capped at 1 MiB; sources at 32 MiB each and 64 MiB total. Existing transcript retention remains authoritative.",
    arguments: [sourcePath,
      { name: "--related", type: "string", description: "JSON array of up to 16 exact absolute child rollout paths; no automatic discovery. Default: []." },
      { name: "--limit", type: "integer", minimum: 1, maximum: 50, default: 20, description: "Maximum assignment attempts returned." },
      { name: "--offset", type: "integer", minimum: 0, default: 0, description: "Skip this many attempts; pagination is an observation, not an atomic snapshot." }, json],
  },
  {
    name: "routing-receipt", summary: "Validate a concise routing decision or acceptance receipt for native history",
    audience: "agent", mutates: false, x_output: "json",
    guidance: "Returns routing_receipt as structured tool output; writes no log, index or other state. Emit it in its own tool call; CLI/programmatic wrappers must return only that result, without other commands or extra text. The calling harness may retain the tool result in its existing transcript. Capture a short rationale, never hidden reasoning or private source bodies. A decision needs schema_version:1, kind:decision, a unique decision_id, action (direct|delegate|reuse|escalate), task and reason (600 chars each), requested {model,effort,context,service_tier} (strings or null for unset/inherited), evidence_refs (up to 8 strings), and target for delegated/reused work (exact spawn task_name or followup target). Optional policy_ref names the role/policy revision. An acceptance uses schema_version:1, kind:acceptance, the same decision_id, verdict (pending|accepted|rejected), reason, evidence_refs (required nonempty when accepted), and presentation (unknown|pending|presented). The parent records its own acceptance; this tool does not verify the evidence or approve work. Emit a decision before its action in the same turn; emit acceptance after checking evidence. Keep incomplete/unjoined receipts visible. No separate retention, archive, inference or execution action occurs.",
    arguments: [{ name: "--receipt", type: "string", required: true, description: "JSON object following the decision or acceptance schema above; maximum 8 KiB. Unknown fields are rejected." }, json],
  },
  {
