...
export function decodeAgentchatsConfig(value: unknown): AgentchatsConfig {
  ...
  onlyKeys(root, ["auxiliary"], "config");
  if (root["auxiliary"] === undefined) return EMPTY_CONFIG;
  ...
  if (rawOriginators === undefined) return EMPTY_CONFIG;
  ...
  return { auxiliaryCodexOriginators: originators };
}
